...
class EndingPhase(private val game: ChristmasGame, private val textFormatter: ChristmasTextFormatter) : GamePhase("ending") {
    ...
    override fun start() {
        addTrait(DisplayLeaderboardPhaseTrait(game, textFormatter, game.leaderboard))
        startCountdown()
    }
    ...
    private fun teleportPlayersToLobby() {
        game.broadcast { it.setInstance(MinestomServer.instanceContainer, MinestomServer.world.spawnPos) }
    }
}